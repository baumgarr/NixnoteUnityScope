...
mkdir $package_dir/nixnote-unity-scope/DEBIAN
cp $package_dir/deb/control $package_dir/nixnote-unity-scope/DEBIAN/control

# remove the gpl.txt to avoid a lintian warning
#rm $package_dir/nixnote-scope/usr/share/nixnote2/gpl.txt

# edit the version & architecture
#sed -i "s/__ARCH__/$arch/" $package_dir/nixnote2/DEBIAN/control
sed -i "s/__VERSION__/$version/" $package_dir/nixnote-unity-scope/DEBIAN/control

# build the deb
dpkg -b $package_dir/nixnote-unity-scope $package_dir/nixnote-unity-scope-${version}_noarch.deb

# remove the temporary directory
rm -rf $package_dir/nixnote-unity-scope/DEBIAN
